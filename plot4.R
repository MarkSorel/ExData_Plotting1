...
# 2 Rows and 2 Cols for plotting
par(mfrow=c(2,2))

# Using the plot function and displaying 4 images in 1.
with(subpower_date, {
  plot(subpower_date$Time, as.numeric(as.character(subpower_date$Global_active_power)), type="l",  xlab="",ylab="Global Active Power")  
  plot(subpower_date$Time, as.numeric(as.character(subpower_date$Voltage)), type="l", xlab="datetime", ylab="Voltage")
  ...
  plot(subpower_date$Time,as.numeric(as.character(subpower_date$Global_reactive_power)),type="l", xlab="datetime", ylab="Global_reactive_power")
})
